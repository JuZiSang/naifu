...
export default function EditorTitle({ menuVisible, infoVisible }: EditorTitleProps): JSX.Element {
    ...
    const containerRef = useCallback(
        (node: HTMLDivElement | null) => {
            if (node !== null) {
                ...
                setRightSpace(window.width < 1200 || (!infoVisible && window.width - rect.right < 80))
                setLeftSpace(window.width < 1200 || (!menuVisible && rect.left < 80))
            }
        },
        [infoVisible, menuVisible, window.width]
    )
    ...
    return (
        <TitleWrapper
            ...
            ref={containerRef}
            style={{
                paddingBottom: session.settings.showStoryTitle ?? true ? '15px' : '5px',
            }}
        >
            {(session.settings.showStoryTitle ?? true) && window.width > 500 ? (
                <TitleContainer>
                    <div>
